...
export default function RoutinesForm() {
    ...
    const structures = {
      ...
      3: [
        { dia: "Día 1 (Empuje)", bloques: [
          { label: "PECHO", pools: ["pecho_plano","pecho_inclinado","pecho_declinado"], count: vol(3,2) },
          { label: "HOMBRO LATERAL Y FRONTAL", pools: ["hombro_lateral"], count: vol(2,1) },
          { label: "TRÍCEPS", pools: ["triceps"], count: vol(2,1) }
        ]},
        { dia: "Día 2 (Tirón)", bloques: [
          { label: "ESPALDA", pools: ["espalda_vertical","espalda_horizontal"], count: vol(4,3) },
      ...
      5: [
        { dia: "Día 1 (Empuje)", bloques: [
          { label: "PECHO", pools: ["pecho_plano","pecho_inclinado","pecho_declinado"], count: vol(4,3) },
          { label: "HOMBROS", pools: ["hombro_press","hombro_lateral"], count: vol(3,2) }
        ]},
        { dia: "Día 2 (Tirón)", bloques: [
          { label: "ESPALDA", pools: ["espalda_vertical","espalda_horizontal"], count: vol(4,3) },
          { label: "BÍCEPS", pools: ["biceps"], count: vol(2,1) },
  ...
  const filterByLesions = (pool, lesionesArray = []) => {
    ...
    return pool.filter(ex => {
      if (Array.isArray(ex.riskyTags)) {
        return !lesionesArray.some(lesion => {
  ...
  const generarRutina = async () => {
    setError(""); setRutina([]); setWarnings([]); setSaveMessage("");
    ...
    layout.forEach((diaInfo) => {
      ...
      diaInfo.bloques.forEach((bloque) => {
        ...
        if (safePool.length < bloque.count && lesionesValidas.length > 0) {
          lesionesValidas.forEach(l => { if (safeGeneral[l]) safePool = [...safePool, ...safeGeneral[l]]; });
          isAdapted = true;
        ...
        for (const ex of candidates) {
          ...
          if (totalExercisesToday >= 7) break;
          if (usedNamesThisDay.has(ex.name)) continue;
          if (usedPatternsThisDay.has(ex.pattern)) continue;
          selectedForGroup.push({ name: ex.name, sets: defaultSets, reps: defaultReps, rest: defaultRest, modified: isAdapted });
        ...
        if (selectedForGroup.length > 0) gruposHoy.push({ grupoLabel: bloque.label, ejercicios: selectedForGroup });
      });
    ...
    try {
      ...
      if (!userId) { setSaveMessage("Error: Tu sesión expiró."); setSaving(false); return; }

      if (hasLesion === "si" && lesiones.length > 0) {
        await supabase.from('medical_alerts').delete().eq('user_id', userId);
        const alertasToInsert = lesiones.map(zona => ({
          user_id: userId,
          name: `Lesión en ${zona.charAt(0).toUpperCase() + zona.slice(1)}`,
          severity: "Media",
          observation: "Reportada automáticamente por el usuario."
        }));
      ...
      const { data: routineData, error: routineError } = await supabase
        .from('routines')
        .insert({ user_id: userId, name: `Plan Estructurado - ${nivelMap[nivel]}`, is_active: false })
        .select().single();
      ...
      for (let dIdx = 0; dIdx < resultado.length; dIdx++) {
        ...
        const { data: dayData, error: dayError } = await supabase
          .from('routine_days').insert({ routine_id: routineId, day_name: day.dia, order_index: dIdx }).select().single();
        if (dayError) throw dayError;

        for (let gIdx = 0; gIdx < day.trabajo.length; gIdx++) {
          const group = day.trabajo[gIdx];
          const { data: blockData, error: blockError } = await supabase
            .from('muscle_blocks').insert({ day_id: dayData.id, muscle_name: group.grupoLabel, order_index: gIdx }).select().single();
          ...
          if (exercisesToInsert.length > 0) await supabase.from('exercises').insert(exercisesToInsert);
        }
      }
  ...
  return (
    <div className="dashboard-container rutinas-wrapper">
      <header className="dashboard-header fade-in">
        <div><p className="greeting">Creación de plan</p><h1 className="user-name">Generar Rutina</h1></div>
      </header>
        ...
        <form className="glass-form" onSubmit={(e) => e.preventDefault()}>
          ...
          {hasLesion === "si" && (
            <div className="lesiones-container slide-up">
              <label className="subtitle-label">Zonas a proteger:</label>
              <div className="chips-grid">
                {[
                  ...
                  <button key={zona.id} type="button"
                    className={`chip-btn ${lesiones.includes(zona.id) ? "active" : ""}`}
                    onClick={() => lesiones.includes(zona.id) ? setLesiones(lesiones.filter(l => l !== zona.id)) : setLesiones([...lesiones, zona.id])}>
                    {zona.label}
                  </button>
                ))}
              </div>
        ...
        <div className="rutina-resultados-container">
          ...
          {warnings.length > 0 && (
            <div className="warning-card slide-up" style={{ animationDelay: "0.25s" }}>
              ...
              <div><strong>Modificaciones clínicas aplicadas:</strong><ul>{warnings.map((w, i) => <li key={i}>{w}</li>)}</ul></div>
            </div>
          )}
          <div className="rutina-dias-grid">
            {rutina.map((d, di) => (
              <section key={di} className="bento-card dia-card slide-up" style={{ animationDelay: `${0.3 + (di * 0.1)}s` }}>
                <div className="dia-header"><h3>{d.dia}</h3></div>
                      ...
                      <ul className="ejercicios-list">
                        {g.ejercicios.map((ex, ei) => (
                          <li key={ei} className={ex.modified ? "ejercicio-item modified" : "ejercicio-item"}>
                            ...
                            {ex.modified && <span className="badge-sust" title="Ejercicio adaptado">Seguro</span>}
                          </li>
                        ))}
                      </ul>
                    </div>
