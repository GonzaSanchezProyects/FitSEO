  ...
  return (
    <div className="nutrition-capsule" style={{ animationDelay: `${delay}s` }}>
      ...
      <div className="capsule-track">
        <div className="capsule-fill glow-effect" style={{ width: `${percentage}%`, backgroundColor: color, boxShadow: `0 0 12px ${shadowColor}` }}></div>
      </div>
    </div>
  );
};
...
const DynamicWeightChart = ({ data, targetWeight, onAddClick }) => {
  ...
  const width = 400, height = 80, padding = 10;
    ...
    <div className="organic-chart-container">
      <div className="weight-header">
        <div>
          ...
          {data.length > 1 && <span className={`weight-trend ${trendClass}`}>{trendArrow} {Math.abs(diff)} kg en total</span>}
        </div>
        <div className="weight-actions-group">
          <div className="weight-goal glass-badge">Meta: {targetWeight} kg</div>
          <button className="add-weight-btn" onClick={onAddClick}><FiPlus /></button>
        </div>
      </div>
      <svg viewBox={`0 0 ${width} ${height + 20}`} className="sparkline-svg" preserveAspectRatio="none">
        ...
        <path d={pathD} fill="none" stroke="#2563EB" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" className="sparkline-path" filter="url(#glow)" />
        {data.length > 0 && <circle cx={coords[coords.length-1].x} cy={coords[coords.length-1].y} r="6" fill="#FFFFFF" stroke="#2563EB" strokeWidth="3" className="sparkline-dot" />}
      </svg>
    </div>
  );
};
...
const Inicio = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const { data: userInfo, error: userError } = await supabase
        .from('users').select('first_name, enabled').eq('id', userId).single();
      ...
      if (!estaActivo) { setIsLoading(false); return; }

      const { data: dietPlan } = await supabase
        .from('diet_plans').select('daily_meals ( breakfast, lunch, snack, dinner )')
        .eq('user_id', userId).eq('is_active', true).single();

      if (dietPlan?.daily_meals) {
        let totalKcal = 0, totalP = 0, totalC = 0, totalF = 0;
        const dayCount = dietPlan.daily_meals.length || 1;
      ...
      const { data: weights } = await supabase
        .from('weight_logs').select('weight, created_at').eq('user_id', userId).order('created_at', { ascending: true });
      if (weights?.length > 0) setWeightHistory(weights);
      else {
      ...
      const { data: logs } = await supabase
        .from('access_logs').select('check_in_time').eq('user_id', userId).gte('check_in_time', startOfWeek.toISOString());
  ...
  if (isLoading) return (
    <main className="dashboard-container">
      <header className="dashboard-header fade-in"><p className="greeting">Cargando progreso...</p></header>
    </main>
  );
    ...
    <main className="dashboard-container">
      ...
      <div className="bento-grid" style={{ filter: !isUserActive ? 'blur(4px)' : 'none', pointerEvents: !isUserActive ? 'none' : 'auto' }}>
        <section className="bento-card col-span-2 slide-up" style={{ animationDelay: "0.1s" }}>
          <h2 className="card-title"><div className="title-icon icon-blue"><FiTrendingUp /></div>Evolución Corporal</h2>
        ...
        <section className="bento-card slide-up" style={{ animationDelay: "0.2s" }}>
          <div className="nutrition-header">
            <h2 className="card-title"><div className="title-icon icon-orange"><FiTarget /></div>Objetivo Diario</h2>
        ...
        <section className="bento-card col-span-2 slide-up" style={{ animationDelay: "0.3s" }}>
          <div className="header-split">
            <h2 className="card-title"><div className="title-icon icon-purple"><FiCalendar /></div>Tu Semana</h2>
            <div className="streak-badge glow-amber"><span className="streak-icon">🔥</span><span className="streak-number">{streak}</span></div>
          </div>
          <WeeklyActivity weekData={weekAttendance} />
        </section>
        <section className="bento-card summary-bento slide-up" style={{ animationDelay: "0.4s" }}>
          <h2 className="card-title"><div className="title-icon icon-green"><FiActivity /></div>Métricas Clave</h2>
      ...
      {!isUserActive && !isLoading && (
        <div className="glass-modal-overlay" style={{ zIndex: 9999, backdropFilter: 'blur(8px)' }}>
          <div className="glass-modal fade-in" style={{ textAlign: 'center', padding: '2rem' }}>
            ...
            <p style={{ color: '#9CA3AF', marginBottom: '1.5rem', lineHeight: '1.5' }}>Tu acceso ha sido bloqueado. Acercate a recepción para regularizar tu situación.</p>
            <button className="btn-primary w-full" onClick={handleLogout} style={{ backgroundColor: '#EF4444', border: 'none' }}>Cerrar Sesión</button>
          </div>
        </div>
      )}
