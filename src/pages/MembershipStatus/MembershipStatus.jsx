...
const MembershipStatus = () => {
    ...
    const fetchMembershipData = async () => {
      try {
        ...
        const { data: pagosData } = await supabase
          ...
          .eq('user_id', userId).order('payment_date', { ascending: false }).limit(10);
  ...
  if (isLoading) return (
    <div className="dashboard-container membership-wrapper">
      <header className="dashboard-header fade-in"><p className="greeting">Verificando suscripción...</p></header>
    </div>
  );
  ...
  return (
    <div className="dashboard-container membership-wrapper">
      <header className="dashboard-header fade-in">
        <div><p className="greeting">Mi Suscripción</p><h1 className="user-name">Estado de Cuota</h1></div>
      </header>
      <section className={`bento-card status-hero-card ${estadoClaseCSS} slide-up`} style={{ animationDelay: "0.1s" }}>
        <div className="status-badge"><span className="status-icon">{iconoEstado}</span><span className="estado-text">{textoEstado}</span></div>
        <div className="monto-gigante"><span className="currency">$</span>{ultimoPago}</div>
        <div className="fechas-info">
          <div className="fecha-box"><span className="fecha-label">Último pago</span><span className="fecha-valor"><FiCheckCircle className="tiny-icon"/> {fechaUltimoPago}</span></div>
          <div className="fecha-box highlight"><span className="fecha-label">Próximo vencimiento</span><span className="fecha-valor"><FiCalendar className="tiny-icon"/> {fechaVencimiento}</span></div>
        </div>
        {isUserActive && (
          <div className="progress-wrapper">
            <div className="progress-labels"><span>Días consumidos</span><span>{porcentajeConsumido}%</span></div>
            <div className="glass-progress-bar"><div className="glass-progress-fill" style={{ width: `${porcentajeConsumido}%` }}></div></div>
          </div>
        )}
      </section>
      <section className="bento-card history-card slide-up" style={{ animationDelay: "0.2s" }}>
        ...
        <div className="payment-list">
          {historialPagos.length > 0 ? historialPagos.map((pago, index) => {
            ...
            return (
              <div key={index} className="payment-row">
                ...
                <div className="payment-details"><span className="payment-title">Pago Mensualidad</span><span className="payment-date">{fechaFormateada}</span></div>
                <div className="payment-amount-status">
                  <span className="payment-amount">${pago.amount}</span>
                  <span className={`payment-pill ${pago.status === 'COMPLETED' ? 'paid' : 'pending'}`}>{pago.status === 'COMPLETED' ? 'Pagado' : pago.status}</span>
                </div>
              </div>
            );
          }) : <p style={{ textAlign: "center", color: "#6B7280", fontSize: "0.9rem", marginTop: "1rem" }}>No hay registros de pagos anteriores.</p>}
        </div>
      </section>
    </div>
  );
