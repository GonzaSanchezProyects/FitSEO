...
const getExerciseInfo = (name) => {
  ...
  if (n.includes("sentadilla")) return exerciseLibrary.sentadilla;
  if (n.includes("prensa")) return exerciseLibrary.prensa;
  if (n.includes("jalon") || n.includes("jalón")) return exerciseLibrary.jalon;
  if (n.includes("remo") || n.includes("row")) return exerciseLibrary.remo;
  if (n.includes("press")) return exerciseLibrary.press;
  if (n.includes("extensi") || n.includes("fondos")) return exerciseLibrary.extensi;
  if (n.includes("curl")) return exerciseLibrary.curl;
  if (n.includes("elevaci") || n.includes("vuelos")) return exerciseLibrary.elevaci;
  if (n.includes("plancha") || n.includes("abdominal")) return exerciseLibrary.plancha;
  if (n.includes("puente") || n.includes("hip")) return exerciseLibrary.puente;
...
const Exercises = () => {
  ...
  const checkWeeklyReset = () => {
    ...
    if (!startDateStr) {
      ...
      const diffDays = Math.abs(now - startDate) / (1000 * 60 * 60 * 24);
      if (diffDays >= 7) {
        setCompleted({});
  ...
  const fetchRoutineFromDB = async () => {
    ...
    try {
      ...
      const { data: routine, error: dbError } = await supabase
        .from('routines')
        .select(`id, name, routine_days ( id, day_name, order_index, muscle_blocks ( id, muscle_name, order_index, exercises ( id, name, sets, reps, video_url ) ) )`)
        .eq('user_id', userId).eq('is_active', true).single();

      if (dbError || !routine) { setPlan(null); setLoadingPlan(false); return; }

      const fetchedPlan = {};
      const days = routine.routine_days.sort((a, b) => a.order_index - b.order_index);
      days.forEach(day => {
        const blocksObj = {};
        day.muscle_blocks.sort((a, b) => a.order_index - b.order_index).forEach(block => {
          blocksObj[block.muscle_name] = block.exercises.map(ex => ({ id: ex.id, name: ex.name, sets: ex.sets, reps: ex.reps, video_url: ex.video_url }));
        });
      ...
      if (!savedDay || !fetchedPlan[savedDay]) {
        if (days.length > 0) { setSelectedDay(days[0].day_name); localStorage.setItem("selectedDay", days[0].day_name); }
      }
    } catch (err) {
  ...
  const getYoutubeUrl = (ex) => {
    ...
    return id ? `https://www.youtube.com/watch?v=${id}` : `https://www.youtube.com/results?search_query=${encodeURIComponent("técnica " + cleanName + " short")}`;
  };
    ...
    return (
      <div className="details-content">
        <div className="text-container">
          ...
          <div className="exercise-tips"><strong>🧠 Claves Técnicas:</strong><ul>{info.tips.map((t, i) => <li key={i}>{t}</li>)}</ul></div>
          <div className="buttons-row">
            <a href={getYoutubeUrl(ex)} target="_blank" rel="noopener noreferrer" className="btn-action btn-youtube"><span>Ver Short/Video</span><YouTubeIcon className="btn-icon" /></a>
            <a href={getEresFitnessUrl(ex.name)} target="_blank" rel="noopener noreferrer" className="btn-action btn-eres"><span>Leer Guía</span><AutoStoriesIcon className="btn-icon" /></a>
          </div>
        </div>
      </div>
    );
  ...
  if (loadingPlan) return (
    <div className="dashboard-container exercises-container">
      <div className="status-state-card fade-in"><div className="spinner-wrapper"><FiRefreshCw className="spinning-icon" /></div><h2>Sincronizando...</h2><p>Obteniendo tu rutina desde la base de datos.</p></div>
    </div>
  );

  if (error) return (
    <div className="dashboard-container exercises-container">
      <div className="status-state-card error-card fade-in"><div className="icon-wrapper error"><FiAlertCircle /></div><h2>Ocurrió un error</h2><p>{error}</p><button className="btn-primary glow-btn mt-2" onClick={fetchRoutineFromDB}>Reintentar</button></div>
    </div>
  );

  if (!plan || Object.keys(plan).length === 0) return (
    <div className="dashboard-container exercises-container">
      <header className="dashboard-header fade-in"><div><p className="greeting">Entrenamiento</p><h1 className="user-name">Sin Rutina</h1></div></header>
      <div className="status-state-card empty-card slide-up">
        <div className="icon-wrapper empty"><FiActivity /></div>
        <h2>Aún no tienes rutina</h2>
        <p>Tu plan de entrenamiento está vacío. Ve a tu perfil y genera uno personalizado.</p>
        <button className="btn-primary glow-btn mt-2" onClick={() => navigate('/rutinesForm')}>Generar mi Rutina</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="dashboard-container exercises-container">
      <header className="dashboard-header fade-in">
        <div><p className="greeting">Seguimiento diario</p><h1 className="user-name">Entrenamiento</h1></div>
      </header>

      <section className="bento-card day-control-panel slide-up">
        <div className="day-control-header">
          <select value={selectedDay} onChange={(e) => { setSelectedDay(e.target.value); localStorage.setItem("selectedDay", e.target.value); }} className="day-selector glass-select">
            {Object.keys(plan).map(d => <option key={d} value={d}>{d}</option>)}
          </select>
          <button onClick={handleManualReset} className="reset-week-btn" title="Reiniciar semana"><FiRefreshCw /></button>
        </div>
        ...
        {Object.entries(exercisesForDay).map(([muscleGroup, groupExs], idx) => (
          <div key={muscleGroup} className="bento-card muscle-block-card slide-up" style={{ animationDelay: `${0.1 + (idx * 0.1)}s` }}>
            ...
            <div className="exercise-cards">
              {groupExs.map((ex) => {
                const isDone = !!completed[selectedDay]?.[ex.id];
                return (
                  <Accordion key={ex.id} className="custom-accordion">
                    <AccordionSummary expandIcon={<ExpandMoreIcon className="expand-icon" />}>
                      <div className={`accordion-header ${isDone ? 'exercise-done' : ''}`}>
                        <div className="checkbox-wrapper" onClick={(e) => { e.stopPropagation(); toggleCompleted(selectedDay, ex.id); }}>
