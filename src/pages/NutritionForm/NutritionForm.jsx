...
const normalizeDays = (days) => {
  ...
  options.forEach(o => { const d = Math.abs(days - o); if (d < bestDiff) { best = o; bestDiff = d; } });
  return best;
};
...
const getSafeFoodForConditions = (food, mealKey, conds, warnSet) => {
  ...
  const conflicts = conds.some(cond => { const rule = SUBSTITUTIONS[cond]; if (!rule) return false; return containsAny(tags, rule.avoidTags || []); });
  if (!conflicts) return { food, modified: false, reason: null };
  for (const cond of conds) {
    const rule = SUBSTITUTIONS[cond]; if (!rule) continue;
...
const pickItemsForMeal = (mealKey, count, conds, usedToday, warnSet) => {
  ...
  while (selected.length < count && i < pool.length) {
    ...
    selected.push(resolved); usedToday.add(resolved.food.name);
  }
...
const createPlan = (conds, daysNum) => {
  ...
  for (let d = 0; d < daysNum; d++) {
    ...
    plan[dayName] = {
      desayuno: pickItemsForMeal("desayuno", countsByMeal.desayuno, conds, usedToday, warnSet),
      almuerzo: pickItemsForMeal("almuerzo", countsByMeal.almuerzo, conds, usedToday, warnSet),
      cena: pickItemsForMeal("cena", countsByMeal.cena, conds, usedToday, warnSet),
      colaciones: pickItemsForMeal("colacion", countsByMeal.colacion, conds, usedToday, warnSet),
    };
  }
...
export default function NutritionForm() {
  ...
  const [formData, setFormData] = useState({ objetivo: "", peso: "", nivelActividad: "", dias: "7", sexo: "", altura: "" });
  ...
  const toggleCondition = (condId) => conditions.includes(condId) ? setConditions(conditions.filter(c => c !== condId)) : setConditions([...conditions, condId]);

  const handleGenerate = async (e) => {
    e?.preventDefault();
    setWarnings([]); setGeneratedPlan(null); setSaveFeedback(null); setSaving(true);
    ...
    setGeneratedPlan(plan); setWarnings(warnings);

    try {
      const { data: authData } = await supabase.auth.getSession();
      const userId = authData?.session?.user?.id;
      if (!userId) { setSaveFeedback({ type: 'error', text: "Error: Tu sesión expiró o no estás logueado." }); setSaving(false); return; }

      if (hasConditions === "si" && conds.length > 0) {
        const condicionesMedicas = conds.filter(c => ['lactosa','celiaca','diabetes','hipertension'].includes(c));
        if (condicionesMedicas.length > 0) {
          const { data: existingAlerts } = await supabase.from('medical_alerts').select('name').eq('user_id', userId);
          const alertasExistentesStr = (existingAlerts || []).map(a => a.name.toLowerCase());
          const nuevasAlertas = [];
          condicionesMedicas.forEach(cond => {
            let nombreAlerta = "", gravedad = "Media";
            ...
            if (!alertasExistentesStr.includes(nombreAlerta.toLowerCase())) {
              nuevasAlertas.push({ user_id: userId, name: nombreAlerta, severity: gravedad, observation: "Reportada automáticamente desde el generador de dietas." });
            }
          });
          if (nuevasAlertas.length > 0) await supabase.from('medical_alerts').insert(nuevasAlertas);
        }
      }
      ...
      const { data: newPlan, error: planError } = await supabase
        .from('diet_plans').insert({ user_id: userId, is_active: true, start_date: new Date().toISOString().split('T')[0] }).select().single();
      if (planError) throw planError;

      const dailyMealsToInsert = Object.entries(plan).map(([dayName, meals]) => ({
        diet_plan_id: newPlan.id, day_name: dayName,
        breakfast: JSON.stringify(meals.desayuno), lunch: JSON.stringify(meals.almuerzo),
        snack: JSON.stringify(meals.colaciones), dinner: JSON.stringify(meals.cena)
      }));
  ...
  const handleClear = () => {
    ...
    setHasConditions("no"); setConditions([]); setGeneratedPlan(null); setWarnings([]); setSaveFeedback(null);
  };

  return (
    <div className="dashboard-container nutrition-form-wrapper">
      <header className="dashboard-header fade-in">
        <div><p className="greeting">Creación de plan</p><h1 className="user-name">Generador de Dieta</h1></div>
      </header>
      ...
      <section className="bento-card slide-up" style={{ animationDelay: "0.1s" }}>
        ...
        <form className="glass-form" onSubmit={(e) => e.preventDefault()}>
          <div className="form-group"><label>Sexo biológico</label>
          ...
          <div className="form-group"><label>Altura (cm)</label><input className="glass-input" type="number" name="altura" value={formData.altura} onChange={handleChange} placeholder="Ej: 175" /></div>
          <div className="form-group"><label>Peso corporal (kg)</label><input className="glass-input" type="number" name="peso" value={formData.peso} onChange={handleChange} placeholder="Ej: 75" /></div>
          <div className="form-group"><label>Objetivo Principal</label>
          ...
          <div className="form-group"><label>Nivel de Actividad Física</label>
          ...
          <div className="form-group"><label>¿Tenés intolerancias o condiciones?</label>
          ...
          {hasConditions === "si" && (
            <div className="lesiones-container slide-up">
              <label className="subtitle-label">Selecciona tus condiciones:</label>
              <div className="chips-grid">
                {[
                  ...
                  <button key={cond.id} type="button" className={`chip-btn ${conditions.includes(cond.id) ? "active" : ""}`} onClick={() => toggleCondition(cond.id)}>{cond.label}</button>
                ))}
              </div>
            </div>
          )}
          <div className="form-group"><label>Estructura del plan</label>
          ...
          <div className="form-actions">
            ...
            <button type="button" className="btn-primary generate-btn" onClick={handleGenerate} disabled={saving}>{saving ? "Procesando..." : "Generar y Guardar"}</button>
          </div>
          {saveFeedback && (
      ...
      {generatedPlan && (
        <div className="rutina-resultados-container slide-up" style={{ animationDelay: "0.2s" }}>
          <div className="resultados-header"><h2 className="section-title">Tu Nuevo Menú</h2></div>
          {warnings.length > 0 && (
            <div className="warning-card">
              <span className="warning-icon">⚠️</span>
              <div><strong>Notas sobre el menú:</strong><ul>{warnings.map((w, i) => <li key={i}>{w}</li>)}</ul></div>
            </div>
          )}
          <div className="rutina-dias-grid">
            {Object.entries(generatedPlan).map(([dia, meals], di) => (
              <section key={dia} className="bento-card dia-card slide-up" style={{ animationDelay: `${0.3 + (di * 0.1)}s` }}>
                <div className="dia-header"><h3>{dia}</h3></div>
      ...
      <div className="back-action slide-up" style={{ animationDelay: "0.8s" }}>
        <button type="button" className="btn-secondary outline" onClick={() => navigate("/")}>Volver al Inicio</button>
      </div>
    </div>
  );
}